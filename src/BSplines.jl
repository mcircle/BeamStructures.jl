using BSplineKit
#b-spline with 5 control points and order 4 (cubic) for curvature interpolation
const KNOTLENGTH = 5
const ORDER = 4
...
function getspline(basis,c,t) #c is the vector of control points, t is the parameter between 0 and 1
    i,bs = evaluate_all(basis,t)
    return sum(c[i:-1:i-ORDER+1] .* bs)
end 
#diff equation for the spline interpolation of the curvature
